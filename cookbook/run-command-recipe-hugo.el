;;; run-command-recipe-hugo.el --- Recipe for Hugo commands -*- lexical-binding: t -*-

;;; Commentary:

;; Recipe for Hugo commands.
...
Honor `<projectroot>/.env' if present."
  (let* ((dotenv-snippet
          (concat "set -a; test -f .env && source .env; set +a;"))
         (project-dir (locate-dominating-file default-directory "archetypes")))
    (when project-dir
      (let ((project-name
        ...
        (list
         (list
          :command-name "hugo:server:with-drafts"
          :command-line
          (concat
           dotenv-snippet
         ...
         (list
          ...
          :working-dir project-dir)
         (list
          :command-name "post:new"
          :command-line (concat dotenv-snippet "hugo new posts/unnamed.md")
          :display "New post"
          :working-dir project-dir))))))

(provide 'run-command-recipe-hugo)
;;; run-command-recipe-hugo.el ends here
